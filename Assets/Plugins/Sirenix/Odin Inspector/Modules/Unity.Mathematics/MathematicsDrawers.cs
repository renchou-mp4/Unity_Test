...
namespace Sirenix.OdinInspector.Modules.UnityMathematics.Editor
{
    ...
    public class Bool2Drawer : OdinValueDrawer<bool2>
    {
        ...
        protected override void DrawPropertyLayout(GUIContent label)
        {
            ...
            Rect contentRect = SirenixEditorGUI.BeginHorizontalPropertyLayout(label, out labelRect);
            {
                ...
                if (label != null) GUILayout.Space(3); // Ugh, better than nothing
    ...
    public class Float2Drawer : OdinValueDrawer<float2>, IDefinesGenericMenuItems
    {
        ...
        protected override void DrawPropertyLayout(GUIContent label)
        {
            ...
            Rect contentRect = SirenixEditorGUI.BeginHorizontalPropertyLayout(label, out labelRect);
            {
                // Slide rect
                {
                    ...
                    if (EditorGUI.EndChangeCheck()) ValueEntry.SmartValue = val;
                }
        ...
        private void SetVector(InspectorProperty property, Vector2 value)
        {
            ...
            {
                for (int i = 0; i < property.ValueEntry.ValueCount; i++) property.ValueEntry.WeakValues[i] = new float2(value.x, value.y);
            });
        }
        ...
        private void NormalizeEntries(InspectorProperty property)
        {
            property.Tree.DelayActionUntilRepaint(() =>
            {
                for (int i = 0; i < property.ValueEntry.ValueCount; i++) property.ValueEntry.WeakValues[i] = math.normalizesafe((float2)property.ValueEntry.WeakValues[i]);
            });
        }
    }
    ...
    public class Float3Drawer : OdinValueDrawer<float3>, IDefinesGenericMenuItems
    {
        ...
        private void SetVector(InspectorProperty property, Vector3 value)
        {
            property.Tree.DelayActionUntilRepaint(() =>
            {
                for (int i = 0; i < property.ValueEntry.ValueCount; i++) property.ValueEntry.WeakValues[i] = new float3(value.x, value.y, value.z);
            });
        }
        ...
        private void NormalizeEntries(InspectorProperty property)
        {
            property.Tree.DelayActionUntilRepaint(() =>
            {
                for (int i = 0; i < property.ValueEntry.ValueCount; i++) property.ValueEntry.WeakValues[i] = math.normalizesafe((float3)property.ValueEntry.WeakValues[i]);
            });
        }
    }
    ...
    public class Float4Drawer : OdinValueDrawer<float4>, IDefinesGenericMenuItems
    {
        ...
        private void SetVector(InspectorProperty property, Vector4 value)
        {
            property.Tree.DelayActionUntilRepaint(() =>
            {
                for (int i = 0; i < property.ValueEntry.ValueCount; i++) property.ValueEntry.WeakValues[i] = new float4(value.x, value.y, value.z, value.w);
            });
        }
        ...
        private void NormalizeEntries(InspectorProperty property)
        {
            property.Tree.DelayActionUntilRepaint(() =>
            {
                for (int i = 0; i < property.ValueEntry.ValueCount; i++) property.ValueEntry.WeakValues[i] = math.normalizesafe((float4)property.ValueEntry.WeakValues[i]);
            });
        }
    }
    ...
    public class Double2Drawer : OdinValueDrawer<double2>, IDefinesGenericMenuItems
    {
        ...
        private void SetVector(InspectorProperty property, Vector2 value)
        {
            property.Tree.DelayActionUntilRepaint(() =>
            {
                for (int i = 0; i < property.ValueEntry.ValueCount; i++) property.ValueEntry.WeakValues[i] = new double2(value.x, value.y);
            });
        }
        ...
        private void NormalizeEntries(InspectorProperty property)
        {
            property.Tree.DelayActionUntilRepaint(() =>
            {
                for (int i = 0; i < property.ValueEntry.ValueCount; i++) property.ValueEntry.WeakValues[i] = math.normalizesafe((double2)property.ValueEntry.WeakValues[i]);
            });
        }
    }
    ...
    public class Double3Drawer : OdinValueDrawer<double3>, IDefinesGenericMenuItems
    {
        ...
        private void SetVector(InspectorProperty property, Vector3 value)
        {
            property.Tree.DelayActionUntilRepaint(() =>
            {
                for (int i = 0; i < property.ValueEntry.ValueCount; i++) property.ValueEntry.WeakValues[i] = new double3(value.x, value.y, value.z);
            });
        }
        ...
        private void NormalizeEntries(InspectorProperty property)
        {
            property.Tree.DelayActionUntilRepaint(() =>
            {
                for (int i = 0; i < property.ValueEntry.ValueCount; i++) property.ValueEntry.WeakValues[i] = math.normalizesafe((double3)property.ValueEntry.WeakValues[i]);
            });
        }
    }
    ...
    public class Double4Drawer : OdinValueDrawer<double4>, IDefinesGenericMenuItems
    {
        ...
        private void SetVector(InspectorProperty property, Vector4 value)
        {
            property.Tree.DelayActionUntilRepaint(() =>
            {
                for (int i = 0; i < property.ValueEntry.ValueCount; i++) property.ValueEntry.WeakValues[i] = new double4(value.x, value.y, value.z, value.w);
            });
        }
        ...
        private void NormalizeEntries(InspectorProperty property)
        {
            property.Tree.DelayActionUntilRepaint(() =>
            {
                for (int i = 0; i < property.ValueEntry.ValueCount; i++) property.ValueEntry.WeakValues[i] = math.normalizesafe((double4)property.ValueEntry.WeakValues[i]);
            });
        }
    }
